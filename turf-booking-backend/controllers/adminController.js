...
// ─── Receptionist Auth ────────────────────────────────────────────────────────
const receptionistRegister = async (req, res) => {
  try {
    const { name, email, phone, password, receptionist_secret_key } = req.body;
    if (!name || !email || !phone || !password || !receptionist_secret_key)
      return res.status(400).json({ message: "All fields are required" });
    if (receptionist_secret_key !== process.env.ADMIN_SECRET_KEY)
      return res.status(403).json({ message: "Invalid secret key" });
    const existing = await User.findOne({ email });
    if (existing) return res.status(400).json({ message: "Email already registered" });
    const salt = await bcrypt.genSalt(10);
    const password_hash = await bcrypt.hash(password, salt);
    const receptionist = await User.create({ name, email, phone, role: "receptionist", auth: { password_hash } });
    const token = generateToken(receptionist._id);
    res.status(201).json({ message: "Receptionist registered", token, user: { id: receptionist._id, name: receptionist.name, email: receptionist.email, role: receptionist.role } });
  } catch (error) { res.status(500).json({ message: "Server error", error: error.message }); }
};
...
const receptionistLogin = async (req, res) => {
  try {
    ...
    if (!user) return res.status(401).json({ message: "Invalid credentials or not a receptionist" });
...
module.exports = { 
  ...
  receptionistRegister, receptionistLogin,
  addTurf, editTurf, deleteTurf, 
