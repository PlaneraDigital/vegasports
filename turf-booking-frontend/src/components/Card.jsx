...
const TurfCard = ({ turf }) => {
  ...
  return (
    <div
      ...
      className="bg-zinc-900 text-white rounded-2xl overflow-hidden shadow-lg cursor-pointer hover:scale-105 transition duration-300 hover:border hover:border-green-600"
    >
      {/* Image */}
      ...
      <div className="p-4">
        ...
        <div className="flex justify-between items-center mt-4">
          <span className="text-green-400 font-bold">
            ₹{turf.price_per_hour}<span className="text-gray-400 font-normal text-xs">/hour</span>
          </span>
          <button
            onClick={(e) => {
              e.stopPropagation()
...
const Card = () => {
  ...
  if (loading) return (
    <div className="text-center text-gray-400 py-10">Loading turfs...</div>
  )
  ...
  if (turfs.length === 0) return (
    <div className="text-center text-gray-400 py-10">No turfs available</div>
  )
