  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 800, color: '#0f172a' }}>Turf Management</h2>
          <p style={{ margin: '0.25rem 0 0', fontSize: '0.8rem', color: '#64748b' }}>{turfs.length} turfs on platform</p>
        </div>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input
              ...
              style={{
                ...
                padding: '0.55rem 0.875rem 0.55rem 2.25rem', color: '#1e293b',
      ...
      {/* Table */}
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 12px rgba(0,0,0,0.03)' }}>
        {loading ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
                  {['Turf', 'Location', 'Surface', 'Sports', 'Price/slot', 'Status', 'Actions'].map(h => (
                    <th key={h} style={{ textAlign: 'left', padding: '1rem 1rem', fontSize: '0.7rem', color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {filtered.map((t, i) => {
                  ...
                  return (
                    <tr key={t._id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #f1f5f9' : 'none', transition: 'background 0.15s' }}
                      onMouseEnter={e => e.currentTarget.style.background = '#f8fafc'}
                      onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                    >
                      <td style={{ padding: '0.875rem 1rem' }}>
                        <div style={{ fontWeight: 700, fontSize: '0.85rem', color: '#0f172a' }}>{t.name}</div>
                        <div style={{ fontSize: '0.7rem', color: '#64748b', marginTop: '0.15rem' }}>{t.turf_type || 'multi-purpose'}</div>
                      </td>
                      <td style={{ padding: '0.875rem 1rem', fontSize: '0.8rem', color: '#475569' }}>
                        {t.location?.city || '—'}
                      </td>
                      <td style={{ padding: '0.875rem 1rem', fontSize: '0.8rem', color: '#475569', textTransform: 'capitalize' }}>
                        {t.surface?.replace('_', ' ') || '—'}
                      </td>
                      <td style={{ padding: '0.875rem 1rem' }}>
                        <div style={{ display: 'flex', gap: '0.3rem', flexWrap: 'wrap' }}>
                          {(t.sports || []).slice(0, 3).map(s => (
                            <span key={s} style={{ background: '#f1f5f9', color: '#475569', border: '1px solid #e2e8f0', borderRadius: '5px', padding: '0.15rem 0.4rem', fontSize: '0.65rem', fontWeight: 600, textTransform: 'capitalize' }}>
                              {s}
                            </span>
                          ))}
      ...
      {deleteId && (
        <div style={{
          ...
          <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '2rem', maxWidth: '380px', width: '90%', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}>
            <div style={{ width: '56px', height: '56px', background: '#fef2f2', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.25rem' }}>
              <Trash2 size={24} color="#dc2626" />
            </div>
            <h3 style={{ margin: '0 0 0.5rem', color: '#0f172a', fontWeight: 800 }}>Delete Turf?</h3>
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <button onClick={() => setDeleteId(null)} style={{ flex: 1, padding: '0.75rem', borderRadius: '10px', background: '#f1f5f9', border: '1px solid #e2e8f0', color: '#475569', fontWeight: 700, cursor: 'pointer' }}>
                Cancel
              </button>
