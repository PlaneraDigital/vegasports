...
const Reports = () => {
  ...
  const inputSt = { background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '0.6rem 0.875rem', color: '#0f172a', fontSize: '0.8rem', outline: 'none' }

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      {/* ── Tabs ── */}
      <div style={{ display: 'flex', gap: '0.5rem', background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '0.4rem', width: 'fit-content', boxShadow: '0 4px 12px rgba(0,0,0,0.03)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ ...sectionStyle, display: 'flex', gap: '0.75rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
              <label style={{ color: '#64748b', fontSize: '0.7rem', fontWeight: 700 }}>Period</label>
              <select value={revPeriod} onChange={e => setRevPeriod(e.target.value)} style={inputSt}>
                <option value="monthly">Monthly</option>
                <option value="daily">Daily</option>
              </select>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
              <label style={{ color: '#64748b', fontSize: '0.7rem', fontWeight: 700 }}>Year</label>
              <input type="number" value={revYear} onChange={e => setRevYear(e.target.value)} min="2020" max="2030" style={{ ...inputSt, width: '90px' }} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
              <label style={{ color: '#64748b', fontSize: '0.7rem', fontWeight: 700 }}>Turf (optional)</label>
              <select value={revTurf} onChange={e => setRevTurf(e.target.value)} style={{ ...inputSt, maxWidth: '180px' }}>
          ...
          {revData && (
            <>
              ...
              <h3 style={{ margin: '0 0 1.25rem', fontSize: '0.875rem', fontWeight: 800, color: '#0f172a' }}>
                  Revenue Trend ({revPeriod})
                </h3>
                {revData.data?.length === 0 ? (
                  <div style={{ height: '240px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94a3b8', fontSize: '0.85rem', fontWeight: 600 }}>No data for selected range</div>
                ) : (
      ...
      {activeTab === 'peak' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div style={{ ...sectionStyle, display: 'flex', gap: '0.75rem', alignItems: 'flex-end' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
              <label style={{ color: '#64748b', fontSize: '0.7rem', fontWeight: 700 }}>Turf (optional)</label>
              <select value={peakTurf} onChange={e => setPeakTurf(e.target.value)} style={inputSt}>
                <option value="">All Turfs</option>
                {turfs.map(t => <option key={t._id} value={t._id}>{t.name}</option>)}
              </select>
            </div>
          ...
          {peakData && (
            <div style={sectionStyle}>
              <h3 style={{ margin: '0 0 1.25rem', fontSize: '0.875rem', fontWeight: 800, color: '#0f172a' }}>Bookings by Start Time</h3>
              {peakData.peak_hours?.length === 0 ? (
                <div style={{ textAlign: 'center', color: '#94a3b8', padding: '2rem', fontSize: '0.875rem', fontWeight: 600 }}>No data yet</div>
              ) : (
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={peakData.peak_hours?.slice(0, 16)} layout="vertical" margin={{ top: 4, right: 24, left: 20, bottom: 0 }}>
                    ...
                    <YAxis type="category" dataKey="start_time" tick={{ fill: '#0f172a', fontSize: 11, fontWeight: 700 }} axisLine={false} tickLine={false} width={52} />
                    <Tooltip content={<CustomTooltip />} cursor={{ fill: '#f8fafc' }} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {turfBreakdown && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              {/* Pie chart */}
              <div style={sectionStyle}>
                <h3 style={{ margin: '0 0 1.25rem', fontSize: '0.875rem', fontWeight: 800, color: '#0f172a' }}>Revenue Share</h3>
                {turfBreakdown.data?.length === 0 ? (
                  <div style={{ textAlign: 'center', color: '#94a3b8', padding: '2rem', fontWeight: 600 }}>No data</div>
                ) : (
                  <ResponsiveContainer width="100%" height={280}>
                    <PieChart>
                      ...
                      <Tooltip formatter={(v) => `₹${v.toLocaleString('en-IN')}`} contentStyle={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '12px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)', color: '#0f172a', fontWeight: 800 }} />
                    </PieChart>
                  </ResponsiveContainer>
                )}
              </div>
              {/* Table */}
              <div style={sectionStyle}>
                <h3 style={{ margin: '0 0 1.25rem', fontSize: '0.875rem', fontWeight: 800, color: '#0f172a' }}>Ranking</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                  {turfBreakdown.data?.map((t, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', padding: '0.75rem 1rem', background: '#f8fafc', border: '1px solid #f1f5f9', borderRadius: '12px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {userAnalytics && (
            <>
              ...
              {/* Top users table */}
              <div style={sectionStyle}>
                <h3 style={{ margin: '0 0 1.25rem', fontSize: '0.875rem', fontWeight: 800, color: '#0f172a' }}>Top Users by Bookings</h3>
                {userAnalytics.analytics?.top_users_by_bookings?.length === 0 ? (
                  <div style={{ textAlign: 'center', color: '#94a3b8', padding: '2rem', fontSize: '0.875rem', fontWeight: 600 }}>No booking data yet</div>
                ) : (
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid #f1f5f9', background: '#f8fafc' }}>
                        {['#', 'User', 'Email', 'Bookings', 'Total Spent'].map(h => (
                          <th key={h} style={{ textAlign: 'left', padding: '0.75rem 1rem', fontSize: '0.7rem', color: '#64748b', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {userAnalytics.analytics.top_users_by_bookings.map((u, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid #f8fafc' }}>
                          <td style={{ padding: '0.875rem 1rem', color: '#64748b', fontSize: '0.8rem', fontWeight: 900 }}>#{i + 1}</td>
