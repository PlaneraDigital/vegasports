...
function ImageGallery({ images, isMobile = false }) {
  ...
  if (!images || images.length === 0) {
    return (
      <div className={`w-full ${isMobile ? 'h-[500px]' : 'h-[400px]'} bg-zinc-200 rounded-none md:rounded-2xl flex items-center justify-center text-zinc-500 border-0 md:border border-zinc-300`}>
        No images available
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      <div className={`relative w-full overflow-hidden ${isMobile ? 'rounded-none' : 'rounded-2xl'} group ${isMobile ? 'border-0' : 'border border-zinc-200'}`}>
        <img
          key={active}
        ...
        {images.length > 1 && (
          <>
            <button onClick={prev}
              className={`absolute left-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white text-zinc-900 p-2.5 rounded-full opacity-0 ${!isMobile ? 'group-hover:opacity-100' : 'md:opacity-0 md:group-hover:opacity-100'} transition-all duration-200 backdrop-blur-md shadow-sm`}>
              <ChevronLeft size={18} />
            </button>
            <button onClick={next}
              className={`absolute right-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white text-zinc-900 p-2.5 rounded-full opacity-0 ${!isMobile ? 'group-hover:opacity-100' : 'md:opacity-0 md:group-hover:opacity-100'} transition-all duration-200 backdrop-blur-md shadow-sm`}>
    ...
    <div className="bg-white md:bg-zinc-50 text-zinc-900 min-h-screen pb-24 md:pb-0">
      ...
      <div className="md:hidden relative">
        ...
        <button onClick={() => navigate(-1)}
          className="absolute top-6 left-5 z-20 flex items-center justify-center w-10 h-10 rounded-full bg-white/90 backdrop-blur-md text-zinc-900 hover:bg-white transition-all shadow-lg">
          <ArrowLeft size={18} />
        </button>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-0 md:px-5 sm:px-8 mt-0 md:mt-8">
        <div className="grid grid-cols-1 md:grid-cols-[1fr_320px] lg:grid-cols-[1fr_360px] gap-0 md:gap-10 lg:gap-14 items-start">
          ...
          <div className="md:hidden">
            {/* Card: Name & Rating */}
            <div className="bg-white border-b border-zinc-100 px-5 py-6">
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="flex items-center gap-1.5 bg-yellow-50 px-3 py-1.5 rounded-lg">
                  ...
                  <span className="text-zinc-900 font-black text-sm">4.9</span>
                </div>
                <span className="text-zinc-500 text-xs font-medium">Google Rating</span>
              </div>

              <button
                onClick={() => window.open('https://maps.google.com/?q=Stephen+Menezes+Marg,+Virar+West', '_blank')}
                className="w-full flex items-start gap-2 text-left mb-4">
                <MapPin size={14} className="text-green-600 flex-shrink-0 mt-1" />
                <span className="text-zinc-600 text-xs font-medium leading-relaxed">Stephen Menezes Marg, near Joyville Shapoorji Pallonji, Bolinj, Virar West, Vasai-Virar, Maharashtra 401303</span>
              </button>

              <button
                onClick={() => window.open('https://maps.app.goo.gl/DugkvuHmQ1L2zFw77', '_blank')}
                className="w-full py-2 bg-green-50 border border-green-200 text-green-700 font-bold rounded-lg text-xs flex items-center justify-center gap-2"
              >
            ...
            {turf.sports?.length > 0 && (
              <div className="bg-white border-b border-zinc-100 px-5 py-5">
                <h3 className="text-sm font-bold text-zinc-900 mb-3 flex items-center gap-2">
                  <Layers size={14} className="text-green-600" /> Sports Available
                </h3>
                <div className="flex flex-wrap gap-2">
                  {turf.sports.map(sport => (
                    <div key={sport} className="flex items-center gap-1.5 bg-green-50 border border-green-200 text-green-700 font-bold py-1.5 px-3 rounded-lg text-xs">
            ...
            {/* Card: Price Info */}
            <div className="bg-white border-b border-zinc-100 px-5 py-5">
              <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-2">Price</p>
              <div className="flex items-baseline gap-1.5">
                <span className="text-3xl font-black text-black">{sym}{turf.price_per_hour}</span>
            ...
            {turf.amenities?.length > 0 && (
              <div className="bg-white border-b border-zinc-100 px-5 py-5">
                <h3 className="text-sm font-bold text-zinc-900 mb-3 flex items-center gap-2">
                  <Zap size={14} className="text-green-600" /> Amenities
                </h3>
                <div className="grid grid-cols-2 gap-2">
                  {turf.amenities.map((a, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs font-medium text-zinc-700 bg-green-50 border border-green-200 px-3 py-2 rounded-lg">
            ...
            {turf.rules?.length > 0 && (
              <div className="bg-white px-5 py-5">
                <h3 className="text-sm font-bold text-zinc-900 mb-3 flex items-center gap-2">
                  <Info size={14} className="text-green-600" /> Turf Rules
                </h3>
                <ul className="space-y-2">
                  {turf.rules.slice(0, 3).map((rule, i) => (
                    <li key={i} className="flex items-start gap-2.5 text-xs text-zinc-600 font-medium">
                      <span className="flex-shrink-0 w-5 h-5 rounded-lg bg-green-50 text-green-700 text-[10px] flex items-center justify-center font-bold">
                        {i + 1}
                      </span>
                      <span>{rule}</span>
                    </li>
          ...
          <div className="hidden md:block">
            ...
            <h1 className="mt-4 text-3xl md:text-4xl font-black tracking-tight text-zinc-900 mb-4">
              {turf.name}
            </h1>
            ...
            <div className="bg-white border border-zinc-200 rounded-2xl p-5 mb-6">
              <div className="flex items-center gap-3 mb-4 pb-4 border-b border-zinc-100">
                {/* <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-zinc-900 font-black text-lg">4.9</span>
                </div> */}
                {/* <span className="text-zinc-500 text-sm font-medium">Google Rating</span> */}
              </div>

              <div className="mb-4">
                <p className="text-zinc-500 text-xs uppercase font-bold tracking-wider mb-2">Location</p>
                <p className="flex items-start gap-2 text-zinc-700 font-medium text-sm leading-relaxed">
                  <MapPin size={16} className="text-green-600 flex-shrink-0 mt-0.5" />
                  <span>Stephen Menezes Marg, near Joyville Shapoorji Pallonji, Bolinj, Virar West, Vasai-Virar, Maharashtra 401303</span>
                </p>
              </div>

              <button
                onClick={() => window.open('https://maps.app.goo.gl/DugkvuHmQ1L2zFw77', '_blank')}
                className="w-full mt-4 py-2.5 bg-green-50 border border-green-200 text-green-700 font-bold rounded-lg hover:bg-green-100 transition-colors flex items-center justify-center gap-2 text-sm"
              >
            ...
            {turf.sports?.length > 0 && (
              <div className="mb-6">
                <h3 className="text-base font-bold text-zinc-900 mb-3 flex items-center gap-2">
                  <Layers size={15} className="text-green-600" /> Sports Available
                </h3>
                <div className="flex flex-wrap gap-3">
                  {turf.sports.map(sport => (
                    <div key={sport} className="flex items-center gap-2 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 text-zinc-900 font-bold py-2.5 px-4 rounded-xl shadow-sm">
                      <span className="text-lg">{SPORT_EMOJI[sport] || "🏅"}</span>
                      <span className="uppercase text-sm">{sport}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="hidden md:block w-full">
            <div className="sticky top-24">
              <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm p-6 space-y-6">
                ...
                {/* Price Info */}
                <div>
                  <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-2">Price Starting From</p>
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-4xl font-black text-black">{sym}{turf.price_per_hour}</span>
                    <span className="text-zinc-500 text-sm font-medium">/ {turf.slot_duration_minutes} mins</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Google Rating</p>
                    <p className="text-lg font-black text-zinc-900 leading-tight">4.9 / 5.0</p>
                  </div>
                </div>
